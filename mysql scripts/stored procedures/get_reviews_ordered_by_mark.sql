CREATE DEFINER=`root`@`localhost` PROCEDURE `get_reviews_ordered_by_mark`(IN sortOrder VARCHAR(4))
BEGIN
    SET @orderByClause = CONCAT('ORDER BY reviews.review_mark ', sortOrder);
